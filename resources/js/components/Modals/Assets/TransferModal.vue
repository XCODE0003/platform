...
function selectFrom(bill) {
    ...
    if (toBill.value && !eligibleTargets.value.find((b) => b.id === toBill.value.id)) {
        toBill.value = null;
        form.to_bill_id = '';
    }
}

function selectTo(bill) {
    toBill.value = bill;
...
function submit() {
    form.post('/assets/bill/transfer', {
        preserveScroll: true,
        onSuccess: async () => {
            ...
            <form @submit.prevent="submit" class="transfer-form">
                ...
                <div class="pb20">
                    ...
                    <div class="currency-selector">
                        <div
                            class="simple-select"
                            :class="{ open: toOpen, error: form.errors.to_bill_id, disabled: !fromBill }"
                            @click.stop="fromBill && (toOpen = !toOpen, fromOpen = false)"
                        >
                            <div v-if="toBill" class="selected-info">
                                <div class="item-text">
                                    <span class="symbol">{{ toBill.name }}</span>
                                    <span class="balance">
                                        {{ Number.parseFloat(toBill.balance ?? '0').toFixed(8) }} {{ toBill.currency?.symbol ?? '' }}
                                        <span v-if="toBill.demo" class="demo-tag">DEMO</span>
                                    </span>
                                </div>
                            </div>
                            <div v-else class="simple-select__placeholder">
                                {{ fromBill ? 'Choose destination bill' : 'Select source bill first' }}
                            </div>
                            <svg class="chevron" width="16" height="16" viewBox="0 0 16 16" fill="none">
                                <path d="M4 6L8 10L12 6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            </svg>
                        ...
                        <transition name="fade">
                            <div v-if="toOpen" class="simple-select__dropdown">
                                <div class="dropdown-list">
                                    <button
                                        v-for="bill in eligibleTargets"
                                        :key="bill.id"
                                        type="button"
                                        class="dropdown-item"
                                        :class="{ active: toBill?.id === bill.id }"
                                        @click.stop="selectTo(bill)"
                                    >
                                        <div class="item-info">
                                            <span class="symbol">
                                                {{ bill.name }}
                                    ...
                                    <p v-if="!eligibleTargets.length" class="dropdown-empty">
                    ...
                    <div v-if="form.errors.to_bill_id" class="error-message text-red">
                        {{ form.errors.to_bill_id }}
                    </div>
                </div>
                ...
                <button
                    ...
                    :disabled="!fromBill || !toBill || !isAmountValid || form.processing"
                >
